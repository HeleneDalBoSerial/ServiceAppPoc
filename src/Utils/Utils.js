...
export const utils = {
  ...
  getCommunicationUserTokenForOneSignalRegistrationToken: async (
    ...
    throw new Error(
      "Failed to get ACS User Acccess token for the given OneSignal Registration Token"
    );
  },
  getOneSignalRegistrationTokenForCommunicationUserToken: async (
    token,
    communicationUserId
  ) => {
    let response = await axios({
  ...
  teamsPopupLogin: async () => {
    ...
    const popupLoginRespoonse = await oAuthObj.loginPopup({
  ...
  joinThread: async (threadId, userId, displayName, token) => {
    const response = await axios({
      url: "addUser",
      method: "POST",
      headers: {
      ...
      data: JSON.stringify({ threadId, userId, displayName, token }),
    });
    if (response.status === 201) {
      return true;
    }
    throw new Error("Failed to add the user to the thread");
  },
  ...
  isParticipantSpotlighted(participantId, spotlightState) {
    ...
    let rtn = spotlightState.find(
      (element) =>
        this.getIdentifierText(element.identifier) ===
        this.getIdentifierText(participantId)
    );
    return !!rtn;
  },
  ...
  getParticipantPublishStates(participantId, publishedStates) {
    ...
    states.isSpotlighted = this.isParticipantSpotlighted(
      participantId,
      publishedStates.spotlight
    );
    states.isHandRaised = this.isParticipantHandRaised(
      participantId,
      publishedStates.raiseHand
    );
    return states;
  },
};
