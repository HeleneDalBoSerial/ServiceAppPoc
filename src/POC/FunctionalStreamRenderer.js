...
export const FunctionalStreamRenderer = forwardRef(
  (
    ...
    )}-${stream.mediaStreamType}-${stream.id}`;
    ...
    const [displayName, setDisplayName] = useState(
      remoteParticipant?.displayName?.trim() ?? ""
    );
    ...
    const createRenderer = async () => {
      if (!renderer) {
        ...
        throw new Error(
          `[App][StreamMedia][id=${stream.id}][createRenderer] stream already has a renderer`
        );
      }
    };
    ...
    const attachRenderer = () => {
      try {
        if (!view.target) {
          throw new Error(
            `[App][StreamMedia][id=${stream.id}][attachRenderer] target is undefined. Must create renderer first`
          );
        } else {
    ...
    const disposeRenderer = () => {
      ...
      if (renderer) {
        ...
        console.warn(
          `[App][StreamMedia][id=${stream.id}][disposeRender] no renderer to dispose`
        );
      }
    };
    ...
    const initializeComponent = async () => {
      ...
      if (
        dominantSpeakerMode &&
        dominantRemoteParticipant !== remoteParticipant
      ) {
        return;
      }
        ...
        >
          <div
            className={`remote-video-container ${
              isSpeaking && !isMuted ? `speaking-border-for-video` : ``
            }`}
            id={videoContainerId}
            ref={videoContainer}
          >
