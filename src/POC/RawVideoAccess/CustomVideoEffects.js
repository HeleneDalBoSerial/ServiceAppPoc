...
export default class CustomVideoEffects extends React.Component {
  ...
  componentWillUnmount() {
    ...
    if (this.bwVideoElem) {
      this.bwCtx.clearRect(
        0,
        0,
        this.bwVideoElem.width,
        this.bwVideoElem.height
      );
      clearTimeout(this.bwTimeout);
      this.bwVideoElem.srcObject.getVideoTracks().forEach((track) => {
        track.stop();
      });
  ...
  async addEffect(e) {
    switch (e.currentTarget.children[0].textContent) {
      case this.isLocal && this.state.buttons?.add?.label: {
        ...
        const { bwStream, bwVideoElem } = this.bwVideoStream(
          localVideoStreamRawStream
        );
  ...
  render() {
    return (
      <div
        className={`custom-video-effects-buttons ${
          this.isLocal ? "outgoing" : "incoming"
        } inline-block`}
      >
        {this.state.buttons &&
