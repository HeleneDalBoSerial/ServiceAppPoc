...
export default class MakeCall extends React.Component {
  ...
  placeCall = async (withVideo) => {
    try {
      ...
      if (
        this.callAgent.kind === CallAgentKind.CallAgent &&
        this.alternateCallerId.value !== ""
      ) {
      ...
      if (identitiesToCall.length > 1) {
        if (
          this.callAgent.kind === CallAgentKind.TeamsCallAgent &&
          this.threadId === ""
        ) {
  ...
  downloadLog = async () => {
    ...
    element.setAttribute(
      ...
      "data:text/plain;charset=utf-8," +
        encodeURIComponent(this.logBuffer.join("\n"))
    );
  ...
  async runPreCallDiagnostics() {
    try {
      ...
      const preCallDiagnosticsResult = await this.callClient
        .feature(Features.PreCallDiagnostics)
        .startTest(this.tokenCredential);

      const deviceAccess = await preCallDiagnosticsResult.deviceAccess;
      this.setState({
        preCallDiagnosticsResults: {
          ...this.state.preCallDiagnosticsResults,
          deviceAccess,
        },
      });

      const deviceEnumeration =
        await preCallDiagnosticsResult.deviceEnumeration;
      this.setState({
        preCallDiagnosticsResults: {
          ...this.state.preCallDiagnosticsResults,
          deviceEnumeration,
      ...
      const inCallDiagnostics =
        await preCallDiagnosticsResult.inCallDiagnostics;
      this.setState({
        preCallDiagnosticsResults: {
          ...this.state.preCallDiagnosticsResults,
          inCallDiagnostics,
      ...
      throw new Error(
        "Can't run Pre Call Diagnostics test. Please try again..."
      );
    }
  }
      ...
      <div>
        ...
        <div className="card">
          <div className="ms-Grid">
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 ms-sm6 text-right">
                ...
                <PrimaryButton
                  ...
                  text={`${
                    this.state.showCallSampleCode ? "Hide" : "Show"
                  } code`}
                  onClick={() =>
                    this.setState({
                      showCallSampleCode: !this.state.showCallSampleCode,
                    })
                  }
                ></PrimaryButton>
              </div>
            ...
            <div className="mb-2">
              Having provisioned an ACS Identity and initialized the SDK from
              the section above, you are now ready to place calls, join group
              calls, and receiving calls.
            </div>
            {this.state.showCallSampleCode && (
              <pre>
                <code style={{ color: "#b3b0ad" }}>{callSampleCode}</code>
              </pre>
            )}
            ...
            {this.state.ufdMessages.length > 0 && (
              <MessageBar
                ...
                dismissButtonAriaLabel="Close"
              >
                {this.state.ufdMessages.map((msg, index) => (
                  <li key={index}>{msg}</li>
                ))}
              </MessageBar>
            )}
                ...
                <div>
                  <div className="ms-Grid-row mt-3">
                    ...
                    <div className="call-input-panel mb-5 ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl4">
                      ...
                      <TextField
                        className="mb-3"
                        placeholder={this.createdRoomId}
                        defaultValue={this.createdRoomId}
                        disabled={this.state.call || !this.state.loggedIn}
                        label="Rooms id"
                    ...
                    <div className="call-input-panel mb-5 ms-Grid-col ms-sm12 ms-lg12 ms-xl12 ms-xxl4">
                      ...
                      <div>
                        {" "}
                        Or enter meeting coordinates (Thread Id, Message Id,
                        Organizer Id, and Tenant Id)
                      </div>
                      <TextField
                        disabled={this.state.call || !this.state.loggedIn}
                        label="Thread Id"
            ...
            {this.state.call &&
              this.state.isPreCallDiagnosticsCallInProgress && (
                <div>Pre Call Diagnostics call in progress...</div>
              )}
            {this.state.call &&
              !this.state.callSurvey &&
              !this.state.isPreCallDiagnosticsCallInProgress && (
            ...
            {this.state.incomingCall && !this.state.call && (
              <IncomingCallCard
                ...
                acceptCallMicrophoneUnmutedVideoOff={async () =>
                  await this.getCallOptions({ video: false, micMuted: false })
                }
                acceptCallMicrophoneUnmutedVideoOn={async () =>
                  await this.getCallOptions({ video: true, micMuted: false })
                }
                acceptCallMicrophoneMutedVideoOn={async () =>
                  await this.getCallOptions({ video: true, micMuted: true })
                }
                acceptCallMicrophoneMutedVideoOff={async () =>
                  await this.getCallOptions({ video: false, micMuted: true })
                }
                onReject={() => {
            ...
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 text-right">
                ...
                <PrimaryButton
                  ...
                  onClick={() =>
                    this.setState({
                      showPreCallDiagnosticsSampleCode:
                        !this.state.showPreCallDiagnosticsSampleCode,
                    })
                  }
                ></PrimaryButton>
                  ...
                  <div className="pre-call-grid-container">
                    {this.state.preCallDiagnosticsResults.deviceAccess && (
                      <div className="pre-call-grid">
                        ...
                        <div>
                          <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                            Audio:{" "}
                          </div>
                          <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                        ...
                        <div className="pre-call-grid">
                          ...
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.audio.jitter
                              }
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.audio
                                  .packetLoss
                              }
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.audio.rtt
                              }
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.video.jitter
                              }
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.video
                                  .packetLoss
                              }
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="ms-Grid-col ms-u-sm2 pre-call-grid-panel">
                              {
                                this.state.preCallDiagnosticsResults
                                  .inCallDiagnostics.diagnostics.video.rtt
                              }
                            </div>
                          </div>
                        </div>
        ...
        <div className="card">
          <div className="ms-Grid">
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 ms-sm6 text-right">
                <PrimaryButton
                  ...
                  text={`${
                    this.state.showStreamingSampleCode ? "Hide" : "Show"
                  } code`}
                  onClick={() =>
                    this.setState({
                      showStreamingSampleCode:
            ...
            {this.state.showStreamingSampleCode && (
              <pre>
                <code style={{ color: "#b3b0ad" }}>{streamingSampleCode}</code>
              </pre>
            )}
            <h3>Video - try it out.</h3>
            <div>
              From your current call, toggle your video on and off by clicking
              on the <Icon className="icon-text-xlarge" iconName="Video" />{" "}
              icon. When you start your video, remote participants can see your
              video by receiving a stream and rendering it in an HTML element.
            </div>
            <br></br>
            <h3>Screen sharing - try it out.</h3>
            <div>
              From your current call, toggle your screen sharing on and off by
              clicking on the{" "}
              <Icon className="icon-text-xlarge" iconName="TVMonitor" /> icon.
              When you start sharing your screen, remote participants can see
              your screen by receiving a stream and rendering it in an HTML
              element.
            </div>
          </div>
        </div>
        <div className="card">
          <div className="ms-Grid">
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 ms-sm6 text-right">
                <PrimaryButton
                  ...
                  text={`${
                    this.state.showMuteUnmuteSampleCode ? "Hide" : "Show"
                  } code`}
                  onClick={() =>
                    this.setState({
                      showMuteUnmuteSampleCode:
            ...
            {this.state.showMuteUnmuteSampleCode && (
              <pre>
                <code style={{ color: "#b3b0ad" }}>{muteUnmuteSampleCode}</code>
              </pre>
            )}
            <h3>Try it out.</h3>
            <div>
              From your current call, toggle your microphone on and off by
              clicking on the{" "}
              <Icon className="icon-text-xlarge" iconName="Microphone" /> icon.
              When you mute or unmute your microphone, remote participants can
              receive an event about wether your micrphone is muted or unmuted.
            </div>
          </div>
        </div>
        <div className="card">
          <div className="ms-Grid">
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 ms-sm6 text-right">
                <PrimaryButton
                  ...
                  text={`${
                    this.state.showHoldUnholdSampleCode ? "Hide" : "Show"
                  } code`}
                  onClick={() =>
                    this.setState({
                      showHoldUnholdSampleCode:
            ...
            {this.state.showHoldUnholdSampleCode && (
              <pre>
                <code style={{ color: "#b3b0ad" }}>{holdUnholdSampleCode}</code>
              </pre>
            )}
            <h3>Try it out.</h3>
            <div>
              From your current call, toggle hold call and unhold call on by
              clicking on the{" "}
              <Icon className="icon-text-xlarge" iconName="Play" /> icon. When
              you hold or unhold the call, remote participants can receive other
              participant state changed events. Also, the call state changes.
            </div>
          </div>
        </div>
        <div className="card">
          <div className="ms-Grid">
            <div className="ms-Grid-row">
              ...
              <div className="ms-Grid-col ms-lg6 ms-sm6 text-right">
                <PrimaryButton
                  ...
                  text={`${
                    this.state.showDeviceManagerSampleCode ? "Hide" : "Show"
                  } code`}
                  onClick={() =>
                    this.setState({
                      showDeviceManagerSampleCode:
            ...
            <h3>Try it out.</h3>
            <div>
              From your current call, click on the{" "}
              <Icon className="icon-text-xlarge" iconName="Settings" /> icon to
              open up the settings panel. The DeviceManager is used to select
              the devices (camera, microphone, and speakers) to use across the
              call stack and to preview your camera.
            </div>
          </div>
        </div>
      </div>
